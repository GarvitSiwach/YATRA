...
export async function generateMetadata({ params }: FollowersPageProps): Promise<Metadata> {
  return {
    title: `Followers ${params.id}`
  };
}

export default async function FollowersPage({ params }: FollowersPageProps): Promise<JSX.Element> {
  ...
  const followerIds = Array.from(
    new Set(
      followsPayload.follows
        .filter((follow) => follow.followingId === traveler.id)
  ...
  const followerRows = followerUsers.map((user) => ({
    id: user.id,
    name: user.name,
    email: user.email,
    ...
    <main className="min-h-screen bg-[#FAF9F5] px-6 py-12">
      <div className="mx-auto w-full max-w-4xl space-y-6">
        <header className="space-y-3">
          ...
          <h1 className="font-serif text-5xl text-[#1B1B1F]">
            {traveler.name}
            &apos;s Followers
          </h1>
          <p className="text-sm text-[#1B1B1F]/65">{followerUsers.length} followers</p>
          <Link
